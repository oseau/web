...
@migration()
def set_default_time(db):
    # set the default time to the current timestamp
    db["count"].transform(defaults={"time": "CURRENT_TIMESTAMP"})

@migration()
def set_default_time_for_existing_rows(db):
    # update all existing rows with timestamp set to 1970-01-01
    # this is not really a migration, but a one-time operation to set the time for existing rows
    db.execute("UPDATE count SET time = '1970-01-01 00:00:00' WHERE time IS NULL")
